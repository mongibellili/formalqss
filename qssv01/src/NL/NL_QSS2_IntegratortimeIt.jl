...
#=   if Debug
  ...
  while simt < ft #&& count < 15555
    ...
    sch =@timeit "updatescheduler" updateScheduler(nextStateTime, nextInputTime)
    simt = sch[2]
    index = sch[1]
...
#=     if Debug
    ...
    #shift taylor
    @timeit "forloop differentiate" for k = 1:order #evaluate x at new time, and derivatives...remember coeffs store the over facto also              
...
#=     if Debug
    ...
    if sch[3] == :ST_STATE
      @timeit "state-computenext" computeNextTime(Val(2), index, simt, nextStateTime, x, quantum) #
...
#=       if Debug
      ...
      for i = 1:length(SD[index])
        ...
        if j != 0
          ...
          elapsed = simt - tx[j]
          @timeit "state-integrate state"  if elapsed > 0
 ...
 #=          if Debug
          ...
          @timeit "state-computederivative" computeDerivative(Val(2), j,x, f[j](q,simt + t))
          #@timeit "Recompute next" 
 #=          if Debug
            println("loop $count : x after derivative = ", x)
            println("loop $count : q after derivative = ", q)
          end =#
         @timeit "state-Recomputenext" reComputeNextTime(Val(2), j, simt, nextStateTime, x, q, quantum)
   #=        if Debug
      ...
      @timeit "input-computenextInput" computeNextInputTime(Val(2), index, simt, elapsed, simt + t, f[index], nextInputTime, x, q, quantum)
 #=      if Debug
        println("under inputchange loop $count : nextInputTime after computenextInputtime= ", nextInputTime)
      end =#
      @timeit "input-computederivative" computeDerivative(Val(2), index, x, f[index](q,simt + t))
...
#=       if Debug
      ...
      for i = 1:length(SD[index])
        ...
        if j != 0
          # println("dependency loop $j= ")              
          elapsed = simt - tx[j]
          @timeit "input-integrate state"  if elapsed > 0
            #@timeit "integrate state" 
            x[j].coeffs[1] = x[j](t + elapsed).coeffs[1] #evaluate x at new time only...derivatives get updated next using computeDerivative()
            q[j].coeffs[1] = x[j].coeffs[1]
            tx[j] = simt
   ...
   #=        if Debug
          ...
          #@timeit "compute deriv" 
         # println("under inputchange loop $count : q before derivative = ", q)
          @timeit "input-compute derivative2" computeDerivative(Val(2), j,x, f[j](q,simt + t))
          #@timeit "Recompute next" 
   #=        if Debug
            println("under inputchange loop $count : x after derivative = ", x)
            println("under inputchange loop $count : q after derivative = ", q)
          end =#
          @timeit "input-recomputetime" reComputeNextTime(Val(2), j, simt, nextStateTime, x, q, quantum)
      #=     if Debug
