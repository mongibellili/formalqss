...
function QSS_Solve(prob::NLODEProblem{PRTYPE,T,Z,Y,CS};sparsity::Val{Sparsity}=Val(false),lightness::Val{Lightness}=Val(true),finalTime=1.0::Float64,saveat=0.1::Float64,initialTime=0.0::Float64,dQmin=1e-6::Float64,dQrel=1e-3::Float64,maxErr=Inf::Float64) where {PRTYPE,T,Z,Y,CS,Lightness,Sparsity}    
...
function QSS_Solve(prob::NLODEProblem{PRTYPE,T,Z,Y,CS},::Tuple{SolverType, OrderType};sparsity::Val{Sparsity}=Val(false),lightness::Val{Lightness}=Val(true),finalTime=1.0::Float64,saveat=0.1::Float64,initialTime=0.0::Float64,dQmin=1e-6::Float64,dQrel=1e-3::Float64,maxErr=Inf::Float64) where{PRTYPE,T,Z,Y,CS,SolverType,OrderType,Lightness,Sparsity}    
...
function custom_Solve(prob::NLODEProblem{PRTYPE,T,Z,Y,CS},::Type{Val{Solver}},::Type{Val{Order}},::Val{Sparsity},::Val{Lightness},finalTime::Float64,saveat::Float64,initialTime::Float64,dQmin::Float64,dQrel::Float64,maxErr::Float64) where{PRTYPE,T,Z,Y,CS,Solver,Order,Lightness,Sparsity}
...
function createCommonData(prob::NLODEProblem{PRTYPE,T,Z,Y,CS},::Val{T},::Val{Z},::Val{Order},sizehint::Int,finalTime::Float64,saveat::Float64,initialTime::Float64,dQmin::Float64,dQrel::Float64,maxErr::Float64)where{PRTYPE,T,Z,Y,CS,Order}
    ...
    tx = zeros(T)
    tq =  zeros(T)
    nextEventTime=@MVector zeros(Z)#zeros(Z)
   ...
   #cacheSize=prob.cacheSize
    taylorOpsCache=Array{Taylor0,1}()
    for i=1:CS
...
#= function createSpecialQSS_data(::Val{false},::Val{T},::Val{Order},::Val{sizehint})where{T,Order,sizehint}
    savedVars = Vector{Array{Taylor0}}(undef, T)# has to be vector (not SA) cuz to be resized in integrator
    for i = 1:T
        temparr=Array{Taylor0}(undef, sizehint)
        for j=1:sizehint          
...
function createLiqssData(prob::NLODEProblem{PRTYPE,T,Z,Y,CS},::Val{Sparsity},::Val{T},::Val{Order})where{Sparsity,PRTYPE,T,Z,Y,CS,Order}
