...
function test11()
   ...
   # save_prob_to_model(odeprob,"D://QS_Solver//qss//src//models//InvestigateBProblem.jl","B12") #any location you want
    solmliqss1=QSS_Solve_from_model(B11,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
   # save_Sol(solmliqss1)
...
function test12()
   ...
   # save_prob_to_model(odeprob,"D://QS_Solver//qss//src//models//InvestigateBProblem.jl","B12") #any location you want
    solmliqss1=QSS_Solve_from_model(B12,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.06805253887502617, -0.5248046039821167
    λ1, λ2 = -3.523632227874817, -0.32636777212518275
...
function test13()
    ...
    solmliqss1=QSS_Solve_from_model(B13,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.06805253887502617, -0.5248046039821167
    λ1, λ2 = -3.523632227874817, -0.32636777212518275
...
function test14()
    ...
    solmliqss1=QSS_Solve_from_model(B14,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.06805253887502617, -0.5248046039821167
    λ1, λ2 = -3.523632227874817, -0.32636777212518275
...
function test15()
    ...
    solmliqss1=QSS_Solve_from_model(B15,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.06805253887502617, -0.5248046039821167
    λ1, λ2 = -3.523632227874817, -0.32636777212518275
...
function test21()
    ...
    solmliqss1=QSS_Solve_from_model(B21,odeprob,60.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
...
function test22()
    ...
    solmliqss1=QSS_Solve_from_model(B22,odeprob,60.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.03593893348744506, -1.739061066512555
    λ1, λ2 = -6.856244266050219, -0.04375573394978005
...
function test23()
    ...
    solmliqss1=QSS_Solve_from_model(B23,odeprob,60.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.03593893348744506, -1.739061066512555
    λ1, λ2 = -6.856244266050219, -0.04375573394978005
...
function test24()
    ...
    solmliqss1=QSS_Solve_from_model(B24,odeprob,60.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.03593893348744506, -1.739061066512555
    λ1, λ2 = -6.856244266050219, -0.04375573394978005
...
function test25()
    ...
    solmliqss1=QSS_Solve_from_model(B25,odeprob,60.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.03593893348744506, -1.739061066512555
    λ1, λ2 = -6.856244266050219, -0.04375573394978005
...
function test31()
    ...
    solmliqss1=QSS_Solve_from_model(B31,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
...
function test32()
    ...
    solmliqss1=QSS_Solve_from_model(B32,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.02252283136759291, -1.6649771686324073
    λ1, λ2 = -6.909908674529628, -0.3400913254703717
...
function test33()
    ...
    solmliqss1=QSS_Solve_from_model(B33,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.02252283136759291, -1.6649771686324073
    λ1, λ2 = -6.909908674529628, -0.3400913254703717
...
function test34()
    ...
    solmliqss1=QSS_Solve_from_model(B34,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.02252283136759291, -1.6649771686324073
    λ1, λ2 = -6.909908674529628, -0.3400913254703717
...
function test35()
    ...
    solmliqss1=QSS_Solve_from_model(B35,odeprob,20.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
    u1, u2 = -0.02252283136759291, -1.6649771686324073
    λ1, λ2 = -6.909908674529628, -0.3400913254703717
...
open("qssv01_1stPart.txt","a") do io
    println(io,"qssv01_1stPart")
    println(io,err11)
    println(io,err12)
    println(io,err13)
    println(io,err14)
