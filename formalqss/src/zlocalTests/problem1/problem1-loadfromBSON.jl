...
function test()
  ...
  #  solliqss1=QSS_Solve_from_model(twoVarSys13,odeprob,30.0,mliqss2(),saveat(0.1),0.0,1e-6,1e-3)
 ...
 #plotSol(solliqss1)
 #plotSol(solmliqss1)
 #= sol=solInterpolated(solliqss1,1,0.01,30.0)
 #display(sol)
 #display(plot!(sol[1],sol[2]))
 sol2=solInterpolated(solliqss1,2,0.01,30.0)
 BSON.@save "solutionx1.bson" sol
 BSON.@save "solutionx2.bson" sol2 =#
 #= display(plot!(sol2[1],sol2[2]))
 println("press enter to exit")
 readline()  =#


 BSON.@load "formalAB_sysB41_mliqss2_errorE-3_step0.1_ft1000_Interp.bson" solmliqss2Interp
 plotSol(solmliqss2Interp)
#=  BSON.@load "formalAB_sysB41_mliqss2_errorE-3_step0.1_ft1000.bson" solmliqss2
 plotSol(solmliqss2) =#
