...
function test()
    BSON.@load "formalqss/ref_bson/solVectAdvection_N1000d1_Feagin14e-12.bson" solFeagin14VectorN1000d1
  prob=@NLodeProblem begin
    ...
    err1=getAverageErrorByRefs(solFeagin14VectorN1000d1,solliqss2Interp)
   ...
   #=  solmliqss2=QSS_Solve(prob,mliqss2(),dQmin=1e-5,saveat=0.01,dQrel=1e-5,finalTime=10.0)#
    ...
    err2=getAverageErrorByRefs(solFeagin14VectorN1000d1,solmliqss2Interp)
  ...
  #=   solnliqss2=QSS_Solve(prob,nliqss2(),dQmin=1e-5,saveat=0.01,dQrel=1e-5,finalTime=10.0)#
    ...
    err3=getAverageErrorByRefs(solFeagin14VectorN1000d1,solnliqss2Interp)
    ...
    @show solnmliqss2.totalSteps
    solnmliqss2Interp=solInterpolated(solnmliqss2,0.01)
    err4=getAverageErrorByRefs(solFeagin14VectorN1000d1,solnmliqss2Interp)
   ...
   #=  XLSX.openxlsx("ADR N1000d1.xlsx", mode="w") do xf
      sheet = xf[1]
      sheet["A1"] = "ADR_N1000d1 all solvers_"
